...
export function setupProtocol() {
  ...
  logger.info('Protocol', `设置app://协议处理器，使用资源目录: ${RENDERER_DIST}`)
  ...
  protocol.handle('app', async (request): Promise<any> => {
    ...
    try {
      ...
      if (!fs.existsSync(filePath)) {
        logger.error('Protocol', `文件不存在: ${filePath}，尝试查找其他可能的路径`)
      ...
      logger.debug('Protocol', `成功加载资源: ${filePath}, 大小: ${data.length} 字节, MIME: ${mimeType}`)
