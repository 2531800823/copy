...
export class AutoUpdaterService {
  ...
  private registerEvents() {
    ...
    this.autoUpdater.on('error', (error) => {
      ...
      logger.error('Updater', `更新URL: ${this.autoUpdater.getFeedURL()}`);
    ...
    this.autoUpdater.on('checking-for-update', () => {
      ...
      logger.info('Updater', `更新URL: ${this.autoUpdater.getFeedURL()}`);
    })
    ...
    this.autoUpdater.on('download-progress', (progressObj) => {
      ...
      this.mainApplication
        .getMainWindow()
        ?.webContents
        .send('update-progress', progressObj);
    })
    ...
    setTimeout(() => {
      ...
      logger.info(
        'Updater',
        `强制检查开发更新: ${autoUpdater.forceDevUpdateConfig}`,
      )
      ...
      autoUpdater
        ...
        .then((result) => {
          ...
          if (result && result.updateInfo) {
            ...
            logger.info(
              'Updater',
              `发布时间: ${result.updateInfo.releaseDate}`,
            )
            logger.info(
              'Updater',
              `发布页面: ${result.updateInfo.releaseNotes || '无'}`,
            )
          }
        })
        .catch((err) => {
